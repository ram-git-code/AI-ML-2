...
export const AITutorChat: React.FC<AITutorChatProps> = ({ onStartQuiz }) => {
  ...
  const [learningDepth, setLearningDepth] = useState<'standard' | 'simple' | 'advanced'>('standard');
  ...
  // Clean up TTS speech when leaving or unmounting
  useEffect(() => {
    return () => {
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    let query = rawQuery;
    if (learningDepth === 'simple' && !rawQuery.toLowerCase().includes('simple')) {
      query = `${rawQuery} (Please explain using simple, intuitive real-world analogies)`;
    } else if (learningDepth === 'advanced' && !rawQuery.toLowerCase().includes('in-depth')) {
      query = `${rawQuery} (Please provide rigorous academic formulas and deep conceptual analysis)`;
    }
  ...
  const handleDownloadTranscript = () => {
    ...
    const content = `# AI Tutor Chat Session Transcript
Generated by EduAI Studio (NVIDIA NIM AI)
Date: ${new Date().toLocaleDateString()}

---

${messages.map((m) => `### [${m.role === 'user' ? 'Student' : 'AI Educational Tutor'}]
${m.content}
`).join('\n---\n\n')}
`;
    ...
    link.href = url;
    link.download = `ai_tutor_session_${new Date().toISOString().slice(0, 10)}.md`;
    link.click();
  ...
  const renderMarkdown = (text: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      // Empty lines
      if (!line.trim()) {
        elements.push(<div key={`empty-${index}`} style={{ height: '0.4rem' }} />);
        return;
      }
    ...
    if (inCodeBlock && codeBuffer.length > 0) {
      elements.push(
        <pre key="code-unclosed" className="tutor-code-block">
          <code>{codeBuffer.join('\n')}</code>
        </pre>
      );
    }
    ...
    <div className="tutor-chat-container">
      ...
      <div className="tutor-chat-header">
        ...
        <div className="tutor-header-actions">
          {/* Learning Depth Toggle */}
          <div className="depth-selector-group">
            <button
              className={`depth-btn ${learningDepth === 'standard' ? 'active' : ''}`}
              onClick={() => setLearningDepth('standard')}
              title="Standard balanced academic tutor"
            >
              Standard
            </button>
            <button
              className={`depth-btn ${learningDepth === 'simple' ? 'active' : ''}`}
              onClick={() => setLearningDepth('simple')}
              title="Simplified analogies & intuitive explanations"
            >
              Analogy
            </button>
            <button
              className={`depth-btn ${learningDepth === 'advanced' ? 'active' : ''}`}
              onClick={() => setLearningDepth('advanced')}
              title="Deep in-depth formulas & rigor"
            >
              Advanced
            </button>
          </div>
          ...
          {messages.length > 0 && (
            <>
              <button
                className="tutor-action-header-btn"
                ...
                <div className="tutor-msg-bubble">
                  <div className="tutor-msg-header">
                    ...
                    {msg.role === 'assistant' && msg.content && !msg.isStreaming && (
                      <div className="tutor-msg-bubble-actions">
                        <button
                        ...
                        >
                          {speakingMsgId === msg.id ? (
                            <VolumeX size={14} className="text-amber" />
                          ) : (
                            <Volume2 size={14} />
                          )}
                        </button>
                        ...
                        >
                          {copiedMsgId === msg.id ? <Check size={14} className="text-emerald" /> : <Copy size={14} />}
                        </button>
                      </div>
                    )}
                  </div>
      ...
      <div className="tutor-input-container">
        ...
        <div className="tutor-input-footer">
          ...
          <span>Context-Aware AI Tutor • RAG Question Bank & NVIDIA NIM Integration</span>
        </div>
      </div>
    </div>
  );
